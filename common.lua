...
--[[
convert a number into a rate representation.
E.g. 1230 becomes 1.23K
]]--
function format_rate(val)
	if val > (1000 * 1000 * 1000 * 1000 * 1000) then
		return string.format("%.2fP", val / (1000 * 1000 * 1000 * 1000 * 1000))
	elseif val > (1000 * 1000 * 1000 * 1000) then
		return string.format("%.2fT", val / (1000 * 1000 * 1000 * 1000))
	elseif val > (1000 * 1000 * 1000) then
		return string.format("%.2fG", val / (1000 * 1000 * 1000))
	elseif val > (1000 * 1000) then
		return string.format("%.2fM", val / (1000 * 1000))
		...
		for k,v in sorted_grtable do
			...
			elseif viz_info.value_units == "time" then
				print(extend_string(format_time_interval(v), EXTEND_STRING_SIZE) .. keystr)
                        elseif viz_info.value_units == "rate" then
                                if timedelta > 0 then
                                        -- delta in nanoseconds
                                        pctstr = format_rate((v * 10^9) / timedelta)
                                else
                                        pctstr = "0.00"
                                end
